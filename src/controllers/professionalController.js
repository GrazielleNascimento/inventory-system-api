...
export const createProfessional = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Professional created successfully', newProfessional });
  } catch (error) {
    res
      .status(400)
      .json({ error: 'Failed to create professional', details: error.message });
  }
};
...
export const getProfessionals = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ error: 'Not possible getProfessionals', details: error.message });
  }
};
...
export const updateProfessional = async (req, res) => {
  ...
  try {
    ...
    if (updated) {
      const updatedProfessional = await models.Professional.findByPk(
        req.params.id
      );
    ...
    res
      .status(400)
      .json({ error: 'Failed to update Professional', details: error.message });
  }
};
